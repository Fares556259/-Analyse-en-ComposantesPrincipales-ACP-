...
L'Analyse en Composantes Principales (ACP) est une méthode de la statistique multivariée qui consiste à transformer des variables corrélées en nouvelles variables décorrélées les unes des autres. Ces nouvelles variables sont nommées « composantes principales », ou axes principaux.
...
En complément de la classification hiérarchique (HCPC), nous réalisons ici un partitionnement par la méthode des K-means. Cette méthode permet de valider la stabilité de nos groupes.

## Détermination du nombre optimal de clusters
Nous utilisons la méthode de la Silhouette pour confirmer le choix de $k=3$.

```{r kmeans_opt}
# Utilisation de la méthode de la silhouette pour k-means
fviz_nbclust(res.pca$ind$coord, kmeans, method = "silhouette") +
  theme_minimal() +
  labs(title = "Nombre optimal de clusters (Méthode de la Silhouette)")
```

## Exécution de K-means
Nous fixons $k=3$ pour rester cohérent avec l'HCPC et faciliter la comparaison.
...
**Interprétation :**
Le partitionnement K-means produit des groupes très similaires à ceux de l'HCPC. Cela démontre la robustesse de notre segmentation. Les frontières entre les groupes sont nettes, confirmant que les individus au sein d'un même cluster partagent des perceptions très proches face aux algorithmes de recommandation.
